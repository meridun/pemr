...
- Syncing `.github/` → `.claude/` inside one repo — that is `npm run sync:claude-config`, covered
  by [pemr-agent-skill](../pemr-agent-skill/SKILL.md).
...
- **Generalised form** — what model-repo holds: `<PLACEHOLDERS>` for project specifics, `pemr-`
...
|---|---|---|
| Fill-in | `<TEST_CMD>` → `npm test`; `pemr-` → `acme-` | ignore |
...
2. Apply only **Improvement** rows. Generalise on the way: project specifics → `<PLACEHOLDERS>`
   (add new ones to `sdlc/PROFILE.md` and `docs/Development_SdlcAdoption.md` if SDLC), prefix → `pemr-`, drop project
   names and paths.
...
- **Prefix rename hides real diffs.** Normalise `pemr-`/`<prefix>-` before diffing, or every
...
- [pemr-agent-skill](../pemr-agent-skill/SKILL.md) — skill authoring, `sync:claude-config`,
  meta-drift guard
- [pemr-doc-tiers](../pemr-doc-tiers/SKILL.md) — where ported knowledge belongs (L1/L2/L3)
